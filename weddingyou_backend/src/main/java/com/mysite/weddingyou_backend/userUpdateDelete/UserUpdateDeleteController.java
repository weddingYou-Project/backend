...
public class UserUpdateDeleteController {
	...
	public UserUpdateDelete login(@RequestBody UserUpdateDelete user, HttpSession session) { 
		System.out.println(user);
		UserUpdateDelete loginResult = service.login(user); //loginResult에는 dto가 저장됨
		if(loginResult != null) {
			//로그인 성공
//			session.setAttribute("loginEmail", loginResult.getEmail());		 
	...
	public int updatePassword(@RequestBody UserUpdateDelete user, HttpSession session) { //리액트에서 email과 password란 이름으로 전달하도록
		int res = 0;
    ...
    public int forgotPassword(@RequestBody UserUpdateDelete user) {
